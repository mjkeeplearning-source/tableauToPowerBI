...
- **Algorithm:** Render IR `data_model` to TMDL: `database.tmdl`, `model.tmdl`, one `tables/<name>.tmdl` per table (columns, measures, calculated columns, hierarchies). Relationships in `relationships/`. **Datasource-to-M**: render each `Datasource.pbi_m_connector` with `connection_params{}` substituted, credentials omitted. When `Table.physical_schema` and `Table.physical_table` are set (federated join tables), the M partition uses schema-qualified navigation: `Source{[Schema="<schema>", Item="<table>"]}[Data]` instead of the bare `Source{[Item="<table_name>"]}[Data]`. **TMDL measure indentation**: `expression:` lines are indented at 2 tabs (one level deeper than the `measure` keyword at 1 tab) so the TMDL parser treats them as nested child properties, not siblings. **TMDL relationship format**: each `Relationship` IR object is emitted to `relationships/<id>.tmdl` using dot-notation `fromColumn: Table.Column` / `toColumn: Table.Column` — the `fromTable:` / `toTable:` properties are only valid when relationships are defined *inline* in `model.tmdl`; they are not recognised by the TMDL parser in standalone `relationships/*.tmdl` files and produce an `UnknownKeyword` parse error. `FieldRef.column_id` stores the physical column name only (e.g. `order_id`); `render_relationship()` receives `from_table_name` / `to_table_name` as separate arguments and concatenates them into the dot form. **Parameters** emitted per §5.7 intent (what-if tables, disconnected selectors, hidden constants, formatting switches). **LOD INCLUDE/EXCLUDE per-sheet measures** named `<calc_name>__<sheet>`.
