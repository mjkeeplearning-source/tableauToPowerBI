...
./out/<wb>/
  ...
  SemanticModel/
    ...
    definition/                    # REQUIRED: TMDL files (NOT at SemanticModel/ root)
      ...
      tables/
        <table>.tmdl
      relationships.tmdl           # only when relationships present — single file, all relationships
  stages/
    01_extract.json
...
│   │                   # connection_params for federated datasources are resolved from
│   │                   # named_connections[0] (the upstream physical connection), not the
│   │                   # outer <connection class='federated'> which carries no server/dbname.
│   ├─ tables[]: { id, name, datasource_id, columns[], primary_key?,
│   │              physical_schema?, physical_table? }
│   │             # physical_schema / physical_table populated for federated join relations
...
- **Algorithm:** `tableau/document-api-python` for elements within its coverage; `lxml` against `tableau-document-schemas` XSDs for the rest. Walk: datasources → connections (preserving both the original `<connection>` and any `<extract>` block separately) → columns → calculations → worksheets → dashboards → actions. **Lift worksheet-level table-calc metadata** (compute-using, partitioning, addressing) so it can be associated back to the consuming calc in stage 2. **Detect quick-table-calc pill modifiers** on sheet encodings — record them as pending anonymous-calc candidates. Tier-C objects detected here. **Join predicates extracted from `<object-graph><relationships>`**: `extract_object_graph_relationships()` (in `extract/datasources.py`) walks the top-level `<object-graph>` sibling of `<datasources>`, parses each `<relationship><expression op="=">` for the two logical column names, and emits `relationships[]` in the Stage 1 JSON as `[{left_col, right_col}]` dicts. These are distinct from `<relation type='table'>` entries (which only list physical tables); the `<object-graph>` section is the only place the join predicate (equality column pair) is stored in the `.twb`.
...
- **Algorithm:** Render IR `data_model` to TMDL: `database.tmdl`, `model.tmdl`, one `tables/<name>.tmdl` per table (columns, measures, calculated columns, hierarchies). When relationships are present, all are written to a **single `relationships.tmdl`** alongside `model.tmdl` — the TMDL spec places all relationships in one file, not one file per relationship under a `relationships/` subdirectory. **Datasource-to-M**: render each `Datasource.pbi_m_connector` with `connection_params{}` substituted, credentials omitted. When `Table.physical_schema` and `Table.physical_table` are set (federated join tables), the M partition uses schema-qualified navigation: `Source{[Schema="<schema>", Item="<table>"]}[Data]` instead of the bare `Source{[Item="<table_name>"]}[Data]`. **TMDL measure indentation**: `expression:` lines are indented at 2 tabs (one level deeper than the `measure` keyword at 1 tab) so the TMDL parser treats them as nested child properties, not siblings. **TMDL relationship format**: each `relationship` block uses dot-notation `fromColumn: Table.Column` / `toColumn: Table.Column` — these are the only two valid column-reference properties per the TMDL spec; `fromTable:` / `toTable:` do not exist as standalone properties and produce an `UnknownKeyword` parse error. `FieldRef.column_id` stores the physical column name only (e.g. `order_id`); `render_relationship()` receives `from_table_name` / `to_table_name` as separate arguments and concatenates them. **Parameters** emitted per §5.7 intent (what-if tables, disconnected selectors, hidden constants, formatting switches). **LOD INCLUDE/EXCLUDE per-sheet measures** named `<calc_name>__<sheet>`.
